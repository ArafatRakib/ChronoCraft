...
export const StopwatchCard: React.FC<StopwatchCardProps> = ({
  ...
  return (
    <div 
      className={`relative rounded-2xl transition-all duration-300 border-t-4 border-x border-b ${theme.border} bg-white dark:bg-slate-900/90 shadow-sm hover:shadow-md overflow-hidden flex flex-col justify-between ${
    ...
    >
      ...
      <div className="p-6 text-center flex-1 flex flex-col items-center justify-center">
        <div className="inline-flex items-baseline font-mono tracking-tight font-bold select-none my-2">
          {parseInt(time.hours, 10) > 0 && (
            <>
              <span className="text-3xl sm:text-5xl text-slate-800 dark:text-slate-100">{time.hours}</span>
              <span className="text-xl sm:text-3xl text-slate-400 dark:text-slate-500 mx-0.5">:</span>
            </>
          )}
          <span className="text-4xl sm:text-6xl text-slate-800 dark:text-slate-100">{time.minutes}</span>
          <span className="text-2xl sm:text-4xl text-slate-400 dark:text-slate-500 mx-0.5">:</span>
          <span className="text-4xl sm:text-6xl text-slate-800 dark:text-slate-100">{time.seconds}</span>
          <span className="text-xl sm:text-3xl font-semibold text-slate-400 dark:text-slate-500 ml-1.5">
            .{time.centiseconds}
          </span>
        </div>

        {/* Status Indicator */}
        <div className="flex items-center justify-center gap-1.5 mt-1 text-xs font-medium text-slate-500 dark:text-slate-400">
          <span className={`w-2 h-2 rounded-full ${stopwatch.isRunning ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300 dark:bg-slate-700'}`} />
          <span>{stopwatch.isRunning ? 'Running' : elapsedMs > 0 ? 'Paused' : 'Ready'}</span>
          {stopwatch.laps.length > 0 && (
            <span className="ml-2 text-slate-400">({stopwatch.laps.length} {stopwatch.laps.length === 1 ? 'lap' : 'laps'})</span>
          )}
        </div>
      </div>
      ...
      <div className="px-5 pb-5 pt-1">
        <div className="grid grid-cols-3 gap-2">
          ...
          <button
            ...
            className={`py-2.5 px-3 rounded-xl font-medium text-sm flex items-center justify-center gap-1.5 transition-all active:scale-95 ${
              stopwatch.isRunning
          ...
          <button
            ...
            className="py-2.5 px-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-1.5 text-white shadow-md transition-all active:scale-95 hover:opacity-95"
            style={{ backgroundColor: theme.accentHex }}
          >
            {stopwatch.isRunning ? (
              <>
